...
from llm_grabbing_bringup.utils import load_prompt
import time
from llm_grabbing_bringup.LLM_cache import DiskCache

class LMP:
    """Language Model Program (LMP), adopted from Code as Policies."""
    def __init__(self, name, cfg, fixed_vars, variable_vars, debug=False):
        self._name = name
        self._cfg = cfg
        self._debug = debug
        self._base_prompt = load_prompt(f"{self._cfg['prompt_fname']}.txt")
    ...
    def _cached_api_call(self, **kwargs):
        ...
        assert any([chat_model in kwargs['model'] for chat_model in ['gpt-3.5', 'gpt-4']]), 'model name must be gpt-3.5 or gpt-4'
